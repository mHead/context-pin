...
try {
  if (!process.stdin.isTTY) {
    ...
    if (input) {
      ...
      // Claude Code uses 'trigger' or 'type'; Codex uses 'hook_event_name'
      // Codex matcher receives "startup" or "resume" as matcher context
  ...
  try {
    ...
    if (totalPins > 0) {
      ...
      lines.push(
        ...
        '  /context-pin:add "your decision or constraint"',
        '  /context-pin:list to see all pins',
        '',
      );
    }
    ...
    // Check for updates only on startup (not after compaction)
    if (trigger === 'startup') {
    ...
    process.stdout.write(`📌 Pin plugin: failed to load pins (${err.message}). Run /context-pin:list to check.\n`);
  }
}
